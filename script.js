...
const firebaseConfig = {
    apiKey: "AIzaSyCHZoZSQd1aoN2lcb18BZRQEf6Wenv6Lvg",
    authDomain: "barbearia-vitinho-6c136.firebaseapp.com",
    projectId: "barbearia-vitinho-6c136",
    storageBucket: "barbearia-vitinho-6c136.firebasestorage.app",
    messagingSenderId: "546549475124",
    appId: "1:546549475124:web:897ceb611a023e735b6761",
    measurementId: "G-EWBPKSWBJV"
};
...
let _unsubAppointments, _unsubHistory, _unsubBlockedDates, _unsubBlockedSlots;
let _reconnectTimers = { appointments: null, history: null, blockedDates: null, blockedSlots: null };
...
function _listenAppointments() {
    if (_unsubAppointments) { try { _unsubAppointments(); } catch(e) {} }
    _unsubAppointments = db.collection("appointments").onSnapshot(
        (snapshot) => {
            appointments = [];
            snapshot.forEach((doc) => { appointments.push({ key: doc.id, ...doc.data() }); });
            _setConnected(true);
            if (_reconnectTimers.appointments) { clearTimeout(_reconnectTimers.appointments); _reconnectTimers.appointments = null; }
            _safeDataChange();
        },
        (error) => {
            console.error('[Firebase] appointments:', error.message);
            _setConnected(false);
            if (!_reconnectTimers.appointments) {
                _reconnectTimers.appointments = setTimeout(() => { _reconnectTimers.appointments = null; _listenAppointments(); }, 5000);
            }
        }
    );
}
...
function _listenHistory() {
    if (_unsubHistory) { try { _unsubHistory(); } catch(e) {} }
    _unsubHistory = db.collection("history").onSnapshot(
        (snapshot) => {
            appointmentHistory = [];
            snapshot.forEach((doc) => { appointmentHistory.push({ key: doc.id, ...doc.data() }); });
            _safeDataChange();
        },
...
function _listenBlockedDates() {
    if (_unsubBlockedDates) { try { _unsubBlockedDates(); } catch(e) {} }
    _unsubBlockedDates = db.collection("blockedDates").onSnapshot(
        (snapshot) => {
            blockedDates = [];
            snapshot.forEach((doc) => { blockedDates.push({ key: doc.id, ...doc.data() }); });
            _safeBlockedChange();
        },
...
function _listenBlockedSlots() {
    if (_unsubBlockedSlots) { try { _unsubBlockedSlots(); } catch(e) {} }
    _unsubBlockedSlots = db.collection("blockedTimeSlots").onSnapshot(
        (snapshot) => {
            blockedTimeSlots = [];
            snapshot.forEach((doc) => { blockedTimeSlots.push({ key: doc.id, ...doc.data() }); });
            _safeBlockedChange();
        },
...
function db_appointmentPush(data) {
    db.collection("appointments").add({ ...data, createdAt: new Date().toISOString() })
...
async function cleanupPastAppointments() {
    ...
    try {
        ...
        if (snapshot.empty) { _cleanupRunning = false; return; }

        for (const doc of snapshot.docs) {
            const app = { key: doc.id, ...doc.data() };
            try {
...
function completeAppointment(key) {
    ...
    if (!app) return;
    db.collection('history').add({ ...app, status: 'completed', completedDate: new Date().toISOString() })
        .then(() => db.collection('appointments').doc(key).delete())
...
function cancelAppointment(key) {
    if (confirm('Deseja realmente cancelar este agendamento?')) {
        const app = appointments.find(a => a.key === key);
        if (!app) return;
        db.collection('history').add({ ...app, status: 'cancelled', cancelledDate: new Date().toISOString() })
            .then(() => db.collection('appointments').doc(key).delete())
            .catch(e => console.error(e));
    }
}
...
function blockDate() {
    ...
    if (!date) return;
    db.collection('blockedDates').add({ date, createdAt: new Date().toISOString() });
}

function unblockDate(key) { db.collection('blockedDates').doc(key).delete(); }
...
function blockTimeSlot() {
    ...
    db.collection('blockedTimeSlots').add({ date, time, reason, createdAt: new Date().toISOString() })
        .then(() => { document.getElementById('blockTimeReason').value = ''; });
}

function unblockTimeSlot(key) { db.collection('blockedTimeSlots').doc(key).delete(); }
...
function saveWorkingHours() {
    ...
    db.collection('settings').doc('workingHours').set({ opening, closing, updatedAt: new Date().toISOString() })
        .then(() => alert('✅ Horário salvo!'))
        .catch(e => alert('Erro ao salvar.'));
}

function loadWorkingHours() {
    db.collection('settings').doc('workingHours').get().then(doc => {
        if (doc.exists) {
            const data = doc.data();
            if (data.opening) document.getElementById('openingTime').value = data.opening;
            if (data.closing) document.getElementById('closingTime').value = data.closing;
        }
    });
}
...
function generateTimeSlots(date) {
    ...
    // Horários: intervalos de 40 min das 9h às 20h, com pausa de almoço de 11:40 até 13:40
